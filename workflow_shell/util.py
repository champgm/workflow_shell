...
def run(command: list, vital=True, throw=False):
...
def find_package_jsons():
    click.echo("Finding all package.json files excluding those inside 'node_modules' folders")
    click.echo("This may take a moment...")
    file_list = get_result(["find", ".", "-type", "f", "-name", "package.json", "!", "-path", "*/node_modules/*"])
